...
def train(target_train_dataset:TrainData,target_test_dataset:TestData,source_train_dataset:TrainData,source_test_dataset:TestData,\
    ...
    #!!!对于TapNet由于很很强的距离度量性，所以就不设这个ProbTransfer了
    ...
    input_length_for_cdan = 256   #RandomLayer的输出默认是B*2014的张量，气势上只要input_length_for_cdan是1024的约数即可
                ...
                if with_nvidia:
                    ...
                    str_out = "Epoch:" + str(cur_epoch) + " batch_num:" + str(batch_idx) + " t_c_loss:" + str(
                        ...
                        single_source_classification_loss.data.numpy()) + " s_sl_loss:" + str(s_sl_loss.data.numpy()) +\
                              " t_proto_dist:" + str(proto_dist.data.numpy()) + " s_proto_dist:" + str(source_proto_dist.data.numpy())
    ...
    for cur_epoch in range(epoch_num):
        ...
        for batch_idx in range(rounds_per_epoch):
            ...
            source_to_target_classification_result, source_to_target_before_last_linear = target_classification_module.process_s2t(source_shape_changed_feature)
            #target_classification_module.train()
            ...
            classification_result_s2t2s = source_classification_module.process_s2t2s(source_to_target_before_last_linear)
            ...
            if with_nvidia:
                str_out = "Epoch:" + str(cur_epoch) + " batch_num:" + str(batch_idx) + " t_c_loss:" + str(
                    ...
                    cdan_loss.data.cpu().numpy()) + " s2t2s_c_loss:" + \
                          str(s2t2s_classification_loss.data.cpu().numpy()) + "t_proto_dist:"+\
                          str(proto_dist.data.cpu().numpy())+" s_proto_dist:"+ str(source_proto_dist.data.cpu().numpy())\
                          +" weight_t" + str(weights_grad_norm_t.data.cpu().numpy()) + " weight_s" + \
                ...
                str_out = "Epoch:" + str(cur_epoch) + " batch_num:" + str(batch_idx) + " t_c_loss:" + str(
                    single_classification_loss.data.numpy()) + " t_sl_loss" + str(t_sl_loss.data.numpy()) +\
                          ...
                          + " s2t2s_c_loss:" + str(s2t2s_classification_loss.data.numpy()) + "t_proto_dist:" + \
                          str(proto_dist.data.numpy()) + " s_proto_dist:" + str(source_proto_dist.data.numpy()) \
                          + " weight_t" + str(weights_grad_norm_t.data.numpy()) + " weight_s" + \
                          str(weights_grad_norm_s.data.numpy())
            print(str_out)
