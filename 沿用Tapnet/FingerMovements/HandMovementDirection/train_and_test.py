...
def eval_source_model_traindata(source_feature_extraction_module,source_classification_module,train_dataloader,cur_epoch,with_nvidia=True): #已修改
    predict_list = np.array([])
    label_list = np.array([])
    for i, (x, y) in enumerate(train_dataloader):
        if with_nvidia:
...
def save_source_classification_modules(source_feature_extraction_module, source_classification_module,cur_epoch): #已修改
    torch.save({
        'epoch': cur_epoch,
...
def eval_source_model_being_pretrained(source_feature_extraction_module:DimensionUnification,\
                               source_classification_module,source_dataloader,cur_epoch,whether_test=False,with_nvidia=True): #已修改
    predict_list = np.array([])
    label_list = np.array([])
...
def train(target_train_dataset:TrainData,target_test_dataset:TestData,source_train_dataset:TrainData,source_test_dataset:TestData,\
    ...
    target_train_loader = DataLoader(target_train_dataset, batch_size=50000, shuffle=True)
    source_train_loader = DataLoader(source_train_dataset, batch_size=50000, shuffle=True)
    ...
    #for cur_epoch in range(180):
        ...
        for batch_idx in range(rounds_per_epoch):
            ...
            target_classification_result, proto_dist = target_classification_module((target_feature,target_label,True))
            ...
            if with_nvidia:
                str_out = "Epoch:"+str(cur_epoch)+" batch_num:"+str(batch_idx)+" t_c_loss:"+str(single_classification_loss.data.cpu().numpy())\
                          + " t_sl_loss:"+str(t_sl_loss.data.cpu().numpy()) + " while proto_dist:" + str(proto_dist.data.cpu().numpy())
            else:
                str_out = "Epoch:" + str(cur_epoch) + " batch_num:" + str(batch_idx) + " t_c_loss:" + str(single_classification_loss.data.numpy())\
                          + " t_sl_loss:"+str(t_sl_loss.data.numpy()) + " while proto_dist:" + str(proto_dist.data.numpy())
        ...
        for batch_idx in range(rounds_per_epoch):
            ...
            if with_nvidia:
                str_out = "Epoch:" + str(cur_epoch) + " batch_num:" + str(batch_idx) + " s_c_loss:" + str(
                    single_source_classification_loss.data.cpu().numpy()) + " s_proto_dists:" + str(source_proto_dist.data.cpu().numpy())
            else:
                str_out = "Epoch:" + str(cur_epoch) + " batch_num:" + str(batch_idx) + " s_c_loss:" + str(
                    single_source_classification_loss.data.numpy()) + " s_proto_dists:" + str(source_proto_dist.data.numpy())
    ...
    target_train_loader_batch = DataLoader(target_train_dataset, batch_size=20, shuffle=True)
    source_train_loader_batch = DataLoader(source_train_dataset, batch_size=20, shuffle=True)
    #for cur_epoch in range(1):
    ...
    for cur_epoch in range(epoch_num):
        ...
        if cur_epoch % 20 == 0:
            ...
            for batch_idx in range(rounds_per_epoch):
                ...
                classification_result_s2t2s, s2t2s_proto_dist = source_classification_module.process_s2t2s((transformed_source_to_target_before, source_label))
                single_s2t2s_classification_loss = classification_loss_module(classification_result_s2t2s, source_label) # !!!
                ...
                if with_nvidia:
                    str_out = "Epoch:" + str(cur_epoch) + " batch_num:" + str(batch_idx) + " t_c_loss:" + str(
                        ...
                        cdan_loss.data.cpu().numpy()) + " s2t2s_c_loss:" + \
                              ...
                              str(proto_dist.data.cpu().numpy())+" s_proto_dist:"+ str(source_proto_dist.data.cpu().numpy())\
                    ...
                    str_out = "Epoch:" + str(cur_epoch) + " batch_num:" + str(batch_idx) + " t_c_loss:" + str(
                        single_classification_loss.data.numpy()) + " t_sl_loss" + str(t_sl_loss.data.numpy()) +\
                              ...
                              + " s2t2s_c_loss:" + str(single_s2t2s_classification_loss.data.numpy()) + "t_proto_dist:" + \
