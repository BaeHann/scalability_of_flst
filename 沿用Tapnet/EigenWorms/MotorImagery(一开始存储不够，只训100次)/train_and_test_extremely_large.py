...
from Tapnet_revised import TapNet
...
def eval_model_testdata(target_feature_extraction_module, target_classification_module, test_dataloader, \
                        cur_epoch, with_nvidia=True):  # 已修改
    predict_list = np.array([])
    label_list = np.array([])
    for i, (x, y) in enumerate(test_dataloader):
        if with_nvidia:
            x = x.float().cuda()
            y_predict = target_classification_module((target_feature_extraction_module(x), y, False))
...
def eval_source_model_traindata(source_feature_extraction_module, source_classification_module, train_dataloader,
                                cur_epoch, with_nvidia=True):  # 已修改
    predict_list = np.array([])
    label_list = np.array([])
    for i, (x, y) in enumerate(train_dataloader):
        if with_nvidia:
            x = x.float().cuda()
            y_predict = source_classification_module((source_feature_extraction_module(x), y, False))
            y_predict = y_predict.detach().cpu().numpy()
            y_predict = np.argmax(y_predict, axis=1)
            predict_list = np.concatenate((predict_list, y_predict), axis=0)
            label_list = np.concatenate((label_list, y.numpy()), axis=0)
...
def eval_source_model_testdata(source_feature_extraction_module, source_classification_module, test_dataloader,
...
def save_source_classification_modules(source_feature_extraction_module, source_classification_module,
                                       cur_epoch):  # 已修改
    torch.save({
        'epoch': cur_epoch,
...
def eval_target_model_being_pretrained(target_feature_extraction_module, target_classification_module, \
                                       target_dataloader, cur_epoch, whether_test=False, with_nvidia=True):  # 已修改
    predict_list = np.array([])
    label_list = np.array([])
...
def eval_source_model_being_pretrained(source_feature_extraction_module: DimensionUnification, \
                                       source_classification_module, source_dataloader, cur_epoch, whether_test=False,
                                       with_nvidia=True):  # 已修改
    predict_list = np.array([])
    label_list = np.array([])
...
class Discriminator_ATT(nn.Module):
    ...
    def __init__(self, patch_size, att_hid_dim, depth, heads, mlp_dim):
        ...
        self.transformer1 = Seq_Transformer(patch_size=self.patch_size, dim=att_hid_dim, depth=self.depth,
...
def train(target_train_dataset: TrainData, target_test_dataset: TestData, source_train_dataset: TrainData,
          source_test_dataset: TestData, \
    ...
    """
    if target_original_length > 600:
        dilation = math.floor(target_original_length / 64)
    else:
        dilation = 1
    ...
    target_classification_module = TapNet(target_original_channel, target_original_length, target_num_class, dilation)
    ...
    source_classification_module = TapNet(target_original_channel, target_original_length, source_num_class, dilation)
    # probtrasfer implemented thorugh the transformation of features before Linear
    # !!!原本以为对于TapNet由于很很强的距离度量性，所以就不设这个ProbTransfer了，但是发现根本训不出来
    ...
    input_length_for_cdan = 256  # RandomLayer的输出默认是B*1024的张量，气势上只要input_length_for_cdan是1024的约数即可
    ...
    scheduler_source_classification = torch.optim.lr_scheduler.StepLR(optimizer_source_classification, step_size=25,
                                                                      gamma=0.8)
    scheduler_feature_transfer_between_t_s = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer_feature_transfer_between_t_s, \
    ...
    target_test_loader = DataLoader(target_test_dataset, batch_size=4, shuffle=True)
    source_test_loader = DataLoader(source_test_dataset, batch_size=4, shuffle=True)

    target_classification_module.whole = False
    source_classification_module.whole = False
    target_classification_module.round_num = -1
    source_classification_module.round_num = -1
    ...
    for cur_epoch in range(5):
        ...
        for batch_idx in range(rounds_per_epoch):
            ...
            target_classification_result, proto_dist = target_classification_module(
                (target_feature, target_label, True))
            single_classification_loss = classification_loss_module(target_classification_result, target_label)
            target_classification_loss = single_classification_loss - 0.001 * proto_dist
            if with_nvidia:
                str_out = "Epoch:" + str(cur_epoch) + " batch_num:" + str(batch_idx) + " t_c_loss:" + str(
                    single_classification_loss.data.cpu().numpy()) \
                          + " t_sl_loss:" + str(t_sl_loss.data.cpu().numpy()) + " while proto_dist:" + str(
                    proto_dist.data.cpu().numpy())
            else:
                str_out = "Epoch:" + str(cur_epoch) + " batch_num:" + str(batch_idx) + " t_c_loss:" + str(
                    single_classification_loss.data.numpy()) \
                          + " t_sl_loss:" + str(t_sl_loss.data.numpy()) + " while proto_dist:" + str(
                    proto_dist.data.numpy())
    ...
    for cur_epoch in range(source_epoch_pretrain):
        ...
        for batch_idx in range(rounds_per_epoch):
            ...
            source_classification_result, source_proto_dist = source_classification_module(
                (source_shape_changed_feature, source_label, True))
            single_source_classification_loss = classification_loss_module(source_classification_result, source_label)
            source_classification_loss = single_source_classification_loss - 0.001 * source_proto_dist
            if with_nvidia:
                str_out = "Epoch:" + str(cur_epoch) + " batch_num:" + str(batch_idx) + " s_c_loss:" + str(
                    single_source_classification_loss.data.cpu().numpy()) + " s_proto_dists:" + str(
                    source_proto_dist.data.cpu().numpy())
            else:
                str_out = "Epoch:" + str(cur_epoch) + " batch_num:" + str(batch_idx) + " s_c_loss:" + str(
                    single_source_classification_loss.data.numpy()) + " s_proto_dists:" + str(
                    source_proto_dist.data.numpy())
    ...
    target_train_loader_batch = DataLoader(target_train_dataset, batch_size= 5, shuffle=True)
    source_train_loader_batch = DataLoader(source_train_dataset, batch_size= 5, shuffle=True)

    #for cur_epoch in range(1):
    for cur_epoch in range(80 * target_epoch_pretrain):
        gc.collect()
        torch.cuda.empty_cache()
        target_classification_module.round_num = 0
        source_classification_module.round_num = 0
        target_feature_extraction_module.train()
        target_classification_module.train()
        source_feature_extraction_module.train()
        # source_to_target_feature_trans.train()
        source_classification_module.train()
        SL_CPC.train()
        target_classification_module.refresh = True
        source_classification_module.refresh = True
        target_classification_module.whole = False
        source_classification_module.whole = False
        target_data = list(enumerate(target_train_loader_batch))
        source_data = list(enumerate(source_train_loader_batch))
        # target_data = list(enumerate(target_train_loader))
        # source_data = list(enumerate(source_train_loader))
        ...
        if cur_epoch % 50 == 0:
            ...
            gc.collect()
            torch.cuda.empty_cache()
            #target_classification_module.whole = True
            #source_classification_module.whole = True
            target_data = list(enumerate(target_train_loader))
            rounds_per_epoch = len(target_data)
            for batch_idx in range(rounds_per_epoch):
                gc.collect()
            ...
            for batch_idx in range(rounds_per_epoch):
                ...
                source_classification_result, source_proto_dist = source_classification_module(
                    (source_shape_changed_feature, source_label, True))
                single_source_classification_loss = classification_loss_module(source_classification_result,
                                                                               source_label)
                source_classification_loss = single_source_classification_loss - 0.001 * source_proto_dist
                if with_nvidia:
                    str_out = "Epoch:" + str(cur_epoch) + " batch_num:" + str(batch_idx) + " s_c_loss:" + str(
            ...
            for batch_idx in range(rounds_per_epoch):
                ...
                if with_nvidia:
                    str_out = "Epoch:" + str(cur_epoch) + " batch_num:" + str(batch_idx) + " t_c_loss:" + str(
                        ...
                        single_source_classification_loss.data.cpu().numpy()) \
                              + " s_sl_loss:" + str(s_sl_loss.data.cpu().numpy()) + " t_proto_dist:" + \
                              str(proto_dist.data.cpu().numpy()) + " s_proto_dist:" + str(
                        source_proto_dist.data.cpu().numpy())
                else:
                    str_out = "Epoch:" + str(cur_epoch) + " batch_num:" + str(batch_idx) + " t_c_loss:" + str(
                        single_classification_loss.data.numpy()) \
                              + " t_sl_loss:" + str(t_sl_loss.data.numpy()) + " s_c_loss:" + str(
                        single_source_classification_loss.data.numpy()) + " s_sl_loss:" + str(s_sl_loss.data.numpy()) + \
                              " t_proto_dist:" + str(proto_dist.data.numpy()) + " s_proto_dist:" + str(
                        source_proto_dist.data.numpy())
    ...
    optimizer_weights_s = torch.optim.Adam([weights_grad_norm_s], lr=0.0001)  # 暂时先不用调度器了
    ...
    for cur_epoch in range(epoch_num):
        ...
        if cur_epoch % 20 == 0:
            ...
            for batch_idx in range(rounds_per_epoch):
                ...
                """
                # target_classification_module.eval()  # !!!
                target_classification_module.conv_bn_1.eval()
                target_classification_module.conv_bn_2.eval()
                target_classification_module.conv_bn_3.eval()
                target_classification_module.mapping.eval()
                source_to_target_classification_result, source_to_target_before_last_linear = target_classification_module.process_s2t(
                    source_shape_changed_feature)
                # target_classification_module.train()
                target_classification_module.conv_bn_1.train()
                target_classification_module.conv_bn_2.train()
                target_classification_module.conv_bn_3.train()
                target_classification_module.mapping.train()
                target_classification_result, proto_dist = target_classification_module(
                    (target_feature, target_label, True))
                single_classification_loss = classification_loss_module(target_classification_result, target_label)
                target_classification_loss = single_classification_loss - 0.005 * proto_dist

                source_classification_result, source_proto_dist = source_classification_module(
                    (source_shape_changed_feature, source_label, True))
                single_source_classification_loss = classification_loss_module(source_classification_result,
                                                                               source_label)
                source_classification_loss = single_source_classification_loss - 0.005 * source_proto_dist
                ...
                classification_result_s2t2s, s2t2s_proto_dist = source_classification_module.process_s2t2s(
                    (transformed_source_to_target_before, source_label))
                single_s2t2s_classification_loss = classification_loss_module(classification_result_s2t2s,
                                                                              source_label)  # !!!
                s2t2s_classification_loss = single_s2t2s_classification_loss - 0.005 * s2t2s_proto_dist
                ...
                if with_nvidia:
                    str_out = "Epoch:" + str(cur_epoch) + " batch_num:" + str(batch_idx) + " t_c_loss:" + str(
                        ...
                        cdan_loss.data.cpu().numpy()) + " s2t2s_c_loss:" + \
                              str(single_s2t2s_classification_loss.data.cpu().numpy()) + "t_proto_dist:" + \
                              str(proto_dist.data.cpu().numpy()) + " s_proto_dist:" + str(
                        source_proto_dist.data.cpu().numpy()) \
                              + " s2t2s_proto_dist:" + str(s2t2s_proto_dist.data.cpu().numpy()) + " weight_t" + \
                              str(weights_grad_norm_t.data.cpu().numpy()) + " weight_s" + \
                              str(weights_grad_norm_s.data.cpu().numpy())
                    # 不知道为什么不能在上方用target_nf_loss.data.cpu().numpy()[0]  这个[0]
                else:
                    str_out = "Epoch:" + str(cur_epoch) + " batch_num:" + str(batch_idx) + " t_c_loss:" + str(
                        single_classification_loss.data.numpy()) + " t_sl_loss" + str(t_sl_loss.data.numpy()) + \
                              " s_c_loss:" + str(single_source_classification_loss.data.numpy()) + \
                              " s_sl_loss:" + str(s_sl_loss.data.numpy()) + " cdan_loss:" + str(cdan_loss.data.numpy()) \
                              + " s2t2s_c_loss:" + str(
                        single_s2t2s_classification_loss.data.numpy()) + "t_proto_dist:" + \
                              str(proto_dist.data.numpy()) + " s_proto_dist:" + str(source_proto_dist.data.numpy()) \
                              + " s2t2s_proto_dist:" + str(s2t2s_proto_dist.data.numpy()) + " weight_t" + \
        ...
        if cur_epoch % 2 == 0:
            ...
            save_target_classification_modules(target_feature_extraction_module, target_classification_module,
                                               cur_epoch)
            save_source_classification_modules(source_feature_extraction_module, source_classification_module,
                                               cur_epoch)
            # 一定调成eval模式
            target_feature_extraction_module.eval()
            target_classification_module.eval()
            ...
            with torch.no_grad():
                ...
                eval_source_model_traindata(source_feature_extraction_module, source_classification_module,
                                            source_train_loader, cur_epoch, with_nvidia)
                eval_source_model_testdata(source_feature_extraction_module, source_classification_module,
                                           source_test_loader, cur_epoch, with_nvidia)
            """
            np.save(
